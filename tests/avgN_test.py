...
def main():
    ...
    if write_data:
        createDir(data, replace_query=False)
    ...
    if return_query and np.ndim(test_data)>1:
        ...
        plot = ax.plot_surface(XA, XD, test_z, cmap='bone')
        ...
        plot2 = ax2.contourf(test_z,cmap = 'bone',extent=[min(xD),max(xD),min(xA),max(xA)], levels=50)
