...
class Optimizer:
    ...
    def train(self, initial_chis):
            ...
            for j in range(len(self.vars)):
                ...
                if var_error[j] < self.tol and j in TensorflowParams['train_sites']:
                    ...
                    if var_error_count[j] > 2:
                        if self.Print:print(f'Stopped training because of x{j}_new-x{j}_old =', var_error[j])
        ...
        if self.Print:
            print
            (
                *[f"\nApproximate value of optimal vars: {best_vars[j]}" for j in range(len(self.vars))],
